...
class GameListItem extends React.Component {
	constructor(props) {
		...
		handleNovice(event){
			this.setState((state) => (state.rank[0]) = event.target.value );
		}
	
		handleExpert(event){
			this.setState((state) => (state.rank[1] = event.target.value ));
		}
	
		handleMaster(event){
			this.setState((state) => (state.rank[2] = event.target.value ));
		}
